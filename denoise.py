...
def _run_ffmpeg(
    ...
    Output goes to /tmp/speech2srt-denoise/<path>/.
...
def denoise(path: str) -> list[dict]:
    ...
    intermediate_dir = Path(config.TMP_PREFIX) / path
    ...
    shutil.rmtree(Path(config.TMP_PREFIX) / path, ignore_errors=True)
...
def main(path: str) -> None:
    """CLI entry point: modal run denoise.py --path <path>"""
    results = denoise.remote(path)
